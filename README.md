...
### 4. Add **.env** file
Add .env file with the following varibles declared (fill the missing values):
```python
...
ORACLE_HOST=
ORACLE_PORT=1521
ORACLE_DATABASE="RAC8.WORLD"
ORACLE_DB_TYPE="oracle"
ORACLE_SCHEMA="AIRFLOW"
```
### 5. Install Oracle Instant Client Libraries on Ubuntu
...
```
and then set `LD_LIBRARY_PATH` to ensure that the system can locate the Oracle Instant Client libraries. You can do this by adding the following line to your .bashrc:
```bash
